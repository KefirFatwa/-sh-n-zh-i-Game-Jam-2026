...
@export var texturas_pollo: Array[Texture2D]
...
func _physics_process(delta: float):
	if cocinando and not fin:
		...
		#esto cambia el sprite
		if tiempo_cocinado < (tiempo_meta - margen_error):
			pollo.texture = texturas_pollo[0]
		elif tiempo_cocinado <= (tiempo_meta + margen_error):
			pollo.texture = texturas_pollo[1]
		else:
			pollo.texture = texturas_pollo[2]
